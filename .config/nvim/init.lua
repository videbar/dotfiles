-- General configuration.
require("config")

-- Load Packer plugins.
require("plugins")

-- Key remaps.
require("remaps")

-- Configuration for the different plugins.
require("config_plugins.treesitter")
require("config_plugins.lsp")
require("config_plugins.mason")
require("config_plugins.cmp")
require("config_plugins.luasnip")
require("config_plugins.formatter")
require("config_plugins.telescope")
require("config_plugins.catppuccine")
require("config_plugins.undotree")
require("config_plugins.comment")
require("config_plugins.lualine")
require("config_plugins.autopairs")
require("config_plugins.obsidian")
require("config_plugins.harpoon")
require("config_plugins.surround")
require("config_plugins.miniai")
require("config_plugins.smart_splits")
require("config_plugins.oil")
require("config_plugins.gitsigns")
