...
return {
    ...
    config = function(_, opts)
        ...
        local my_opts = {
            ...
            -- `formatter.filetypes.xxx` defines default configurations for the `xxx`
            -- filetype.
            --
            filetype = {
                -- Formatter configurations for filetype "lua" go here and will be executed
                -- in order.
                lua = {
        ...
        require("formatter").setup(vim.tbl_deep_extend("force", opts, my_opts))
                ...
                if vim.g.format_on_save then
                    ...
                    if #formatters == 0 then
                        ...
                        for _, server in pairs(lsp_servers) do
                            if
                                server.server_capabilities.documentFormattingProvider
                            then
                    ...
                    -- Either way, call `FormatWrite` so that the "any type" formatter
                    -- is run.
                    vim.cmd("FormatWrite")
                end
            end,
